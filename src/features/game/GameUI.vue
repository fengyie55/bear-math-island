...
export default {
  ...
  setup() {
    ...
    function recStat(key, correct, q, spentMs = 0) {
      ...
      if (p.recentAnswers.length > 60) {
        p.recentAnswers = p.recentAnswers.slice(-60)
      }
    ...
    const recommendedLevel = computed(() => {
      ...
      // V6: 优先基于最近作答做轻量自适应推荐
      const recent = (p?.recentAnswers || []).slice(-20)
      if (recent.length >= 8) {
        const levelAgg = {}
        recent.forEach(r => {
          if (!r?.key) return
        ...
        const candidates = Object.entries(levelAgg)
          ...
          // 先按正确率，再按平均耗时（慢且不准优先练）
          .sort((a, b) => (a.acc - b.acc) || (b.avgMs - a.avgMs))

        if (candidates.length > 0) {
          return allLevels.find(l => l.key === candidates[0].key) || null
        }
      }
